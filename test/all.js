...
test('simple replication', async (t) => {
  ...
  const core1 = store1.get({ name: 'foo' })
  await core1.ready()
...
test('replicate multiple top-level cores', async (t) => {
  ...
  const core1 = store1.get({ name: 'foo' })
  await core1.ready()
  const core2 = store1.get({ name: 'bar' })
  await core2.ready()
...
//     flush: true
//   })
//   await networker3.configure(dkey, {
//     announce: false,
//     lookup: true,
//     flush: true
//   })
...
async function create (opts = {}) {
  if (!bootstrap) {
    bootstrap = DHT.bootstrapper(BOOTSTRAP_PORT)
  ...
  const networker = new CorestoreNetworker(store, {
    ...opts
    // bootstrap: [`localhost:${bootstrap.address().port}`]
  })
  return { store, networker }
}

function append (core, data) {
  return core.append(data)
}

function get (core, idx, opts = {}) {
  return core.get(idx, opts)
}
